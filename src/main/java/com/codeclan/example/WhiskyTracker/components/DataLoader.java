...
public class DataLoader implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        ...
        List<Whisky> findWhiskyByYear = whiskyRepository.findWhiskyByYear(2018);

//        List<Distillery> findDistillery = distilleryRepository.findDistilleryByName("Glendronach");

        List<Whisky> found = whiskyRepository.findWhiskyByDistilleryAndAge("Rosebank", 12);

        List<Whisky> SpeysideWhiskeys = whiskyRepository.findAllWhiskyByRegion("Speyside");

        List<Distillery> speysideDistilleries = distilleryRepository.findAllDistilleryByRegion("Speyside");

        List<Distillery> foundAllWhiskies = distilleryRepository.findWhiskiesAgeTwelve(12);
    }
}
